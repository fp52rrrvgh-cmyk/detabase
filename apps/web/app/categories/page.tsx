...
export default function CategoriesPage() {
  ...
  const loadCategories = useCallback(async () => {
    ...
    if (error) {
      setState({ status: "failure", message: "讀取分類失敗" });
      return;
    }
    ...
    <main className="dashboard-shell">
      <div className="dashboard-header">
        <div className="dashboard-header-text">
          ...
          <h1>分類管理</h1>
          <p className="summary">管理你的支出/收入分類</p>
        </div>
      </div>
          ...
          <div className="category-list">
            {state.items.length === 0 ? (
              ...
              state.items.map((cat) => (
                <div
                  key={cat.id}
                  className={`category-row ${!cat.is_active ? "category-row--inactive" : ""}`}
                >
                  {editingId === cat.id ? (
                    ...
                    <>
                      <div className="category-row-info">
                        ...
                        {cat.grouping_purpose ? (
                          <span className="category-purpose">
                            {cat.grouping_purpose}
                          </span>
                        ) : null}
                        {!cat.is_active ? (
                          <span className="category-badge-inactive">已停用</span>
                        ) : null}
                      </div>
                      <div className="category-row-actions">
                        <button
                          className="secondary-button category-action-btn"
                          onClick={() => {
                            setEditingId(cat.id);
                        ...
                        <button
                          className={`secondary-button category-action-btn ${cat.is_active ? "btn-warning" : "btn-success"}`}
                          onClick={() =>
