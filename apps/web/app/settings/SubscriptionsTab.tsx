...
export function SubscriptionsTab() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!sessionData.session) { setError("請先登入"); return; }
    ...
    if (subResult.error) { setError("讀取訂閱失敗"); setLoading(false); return; }

    setItems((subResult.data ?? []) as SubscriptionRow[]);
    setCategories((catResult.data ?? []) as CategoryRef[]);
    setAccounts((acctResult.data ?? []) as AccountRef[]);
    setLoading(false);
  }, [supabase]);
  ...
  async function toggleActive(sub: SubscriptionRow) {
    if (!supabase) return;
    const { error } = await supabase.from("finance_subscriptions").update({ is_active: !sub.is_active }).eq("id", sub.id);
  ...
  async function saveEdit(id: string) {
    ...
    const { error } = await supabase.from("finance_subscriptions").update({
      amount, description: editDesc.trim(), category_id: editCat, account_id: editAcct, frequency: editFreq, next_date: editNext,
    }).eq("id", id);
  ...
  async function addNew() {
    ...
    const { error } = await supabase.from("finance_subscriptions").insert({
      user_id: sessionData.session.user.id, movement_type: newType, amount, category_id: newCat,
      account_id: newAcct, description: newDesc.trim() || (newType === "expense" ? "訂閱支出" : "訂閱收入"),
      frequency: newFreq, next_date: newNext,
    });
    if (error) { setToast("新增失敗"); return; }
    setNewAmount(""); setNewDesc(""); setNewCat(""); setNewAcct(""); setNewNext("");
    setToast("已新增");
    await loadData();
  }
  ...
  return (
    <div>
      <div className="d-header" style={{ marginBottom: 12 }}>
        <div>
          <h3 style={{ margin: 0, fontSize: "1rem", color: "#e2e8f0" }}>🔄 固定訂閱</h3>
        ...
        <>
          ...
          <div className="d-card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "12px 0" }}>
              <div style={{ display: "flex", gap: 8 }}>
                <button className={`d-btn ${newType === "expense" ? "d-btn-active" : ""}`}
                  onClick={() => setNewType("expense")} type="button">支出</button>
                <button className={`d-btn ${newType === "income" ? "d-btn-active" : ""}`}
                  onClick={() => setNewType("income")} type="button">收入</button>
              </div>
          ...
          <div className="d-card">
            ...
            <div className="d-tx-list">
              {items.length === 0 && <p className="status-message status-muted">尚無訂閱項目</p>}
              {items.map((sub) => {
                const isEditing = editingId === sub.id;
                const catName = categories.find((c) => c.id === sub.category_id)?.display_name ?? "未分類";
                const acctName = accounts.find((a) => a.id === sub.account_id)?.display_name ?? "未知";
